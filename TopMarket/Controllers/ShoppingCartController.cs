...
namespace TopMarket.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult VnpayReturn()
        {
            if (Request.QueryString.Count > 0)
            {
                ...
                if (checkSignature)
                {
                    ...
                    {
                        //Payment fail. Error code: vnp_ResponseCode
                        ViewBag.InnerText = "An error occurred. Error code: " + vnp_ResponseCode;
                    }
                    ViewBag.PaymentSuccess = "Payment ammount: " + vnp_Amount.ToString() + " (VND)";
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    order.Status = 1;//Unpaid / 2/Paid, 3/Complete, 4/Candel
                    ...
                    order.TotalAmmount = cart.Items.Sum(x => (x.Price * x.Quantity));
                    order.PaymentMethods = req.PaymentMethods;
                    order.DateCreated = DateTime.Now;
                    order.DateModified = DateTime.Now;
                    order.CreatedBy = req.Phone;
                    Random rd = new Random();
                    order.Code = "OD" + rd.Next(0, 9) + rd.Next(0, 9) + rd.Next(0, 9) + rd.Next(0, 9);
                    db.Orders.Add(order);
                    db.SaveChanges();
                    cart.ClearCart();

                    code = new { Success = true, Code = req.PaymentMethods, Url = "" };
                    //var url = "";
                    if (req.PaymentMethods == 2)
                    {
                        var url = UrlPayment(req.TypePaymentVN, order.Code);
                        code = new { Success = true, Code = req.PaymentMethods, Url = url };
                    }
                }
            }
            return Json(code);
        ...
        public string UrlPayment(int TypePaymentVN, string orderCode)
        {
            ...
            vnpay.AddRequestData("vnp_OrderInfo", "Payment orders :" + order.Code);
