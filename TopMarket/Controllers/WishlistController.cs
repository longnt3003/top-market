...
namespace TopMarket.Controllers
{
	...
	public class WishlistController : Controller
	{
		...
		{
			if (!Request.IsAuthenticated) return Json(new { Success = false, Message = "You need to login first" });

			var checkItem = db.Wishlists.FirstOrDefault(x => x.ProductId == ProductId && x.UserName == User.Identity.Name);
			if (checkItem != null) return Json(new { Success = false, Message = "This product already added to your favorites list." });
